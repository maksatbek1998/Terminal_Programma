...
namespace Kassa.Class
{
    class Treatment
    {
        ...
        public string dalee()
        {
            string id = data.DisplayReturn("SELECT MIN(id) FROM queue_cashier q WHERE user_id IS NULL" +
            ...
            data.SoursData("SELECT q.id,q.queue_number,c.category_suffix,c.id,q.category_id,q.lang FROM " +
                "queue_cashier q JOIN categories c " +
        ...
        public void delete_table()
        {
            ...
            if (hranilishe_stroki[2] == "К") {/*ну типа касса*/}  //условие для кассы

                else
                {
                    data.Registr("insert into queue_users(queue_number, category_id,with_direction,lang) values('" + hranilishe_stroki[1] + "', '" + hranilishe_stroki[4] + "','"+0+"','"+ hranilishe_stroki[5] + "')"); //врачи
                }
        ...
        public void pozvat()
        {
            ...
            string Catid = data.DisplayReturn("SELECT c.id FROM  categories c WHERE c.category_suffix='К'");
